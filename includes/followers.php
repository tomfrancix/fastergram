...
$queryt = "SELECT * FROM users WHERE user_id = {$profileid}";
$select_users_by_idt = mysqli_query($connection, $queryt);
$puser_id = 0;
    $pusername = "";
    $puser_image = "";
    $puser_bio = "";
while($row = mysqli_fetch_assoc($select_users_by_idt)) {
...
<div class="container-fluid">
    <div class="row">
        <label class="form-control" style="color:black;background-color:transparent;border:none;border-bottom:1px solid rgba(100,100,100,0.5);">People who follow <b>@<?php echo $pusername; ?></b></label>
    </div>
<?php
        followuser();
        unfollowuser();
...
$query = "SELECT * FROM following WHERE follow_to_user_id = {$puser_id} ORDER BY follow_id DESC ";
...
$queryQ = "SELECT * FROM users WHERE user_id = {$follow_user_id}";
...
if($ufollow_to_user_id == $user_id) {    $following = true;  ?>
                    ...
                    <a href="connections.php?source=followers&unfollowuser=<?php echo $ufollow_id; ?>&id=<?php echo $puser_id; ?>" type="submit" name="unfollowuser"  style="text-align:center;width:100%;" class="btn btn-default dark">Unfollow</a>
...
if(!$following) { ?>
    <form method="post" action="" >
         ...
         <input type="hidden" name="user_id" value="<?php echo $puser_id; ?>">
                    <button type="submit" name="followuser"  style="text-align:center;width:100%;" class="btn btn-primary">Follow</button>
                </form> <?php
}
                ?>
...
<script> 
    document.getElementById('navbar-header').innerHTML = "Followers";
